...
object Flight {
  ...
  def main(args: Array[String]) {
    ...
    var dfile = "hdfs:///mapr/maprdemo.mapr.io/tmp/flights20170102.json"
    var tfile = "hdfs:///mapr/maprdemo.mapr.io/tmp/flights20170304.json"
    var modeldir = "hdfs:///mapr/maprdemo.mapr.io/tmp/flightmodel"
    ...
    val df: Dataset[Flight] = spark.read.option("inferSchema", "false").schema(schema).json(dfile).as[Flight]
    ...
    val test = spark.read.option("inferSchema", "false").schema(schema).json(tfile).as[Flight]
